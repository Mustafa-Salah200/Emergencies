...
const Details = ({ data, setDetails }) => {
  ...
  const [commentsArray, setCommentsArray] = useState(data && data.comments);

  const HandleActive = async () => {
    active === "active" ? setActive("inactive") : setActive("active");
    const response = await fetch(
      `http://127.0.0.1:4000/api/v1/emergencies/${data._id}`,
      {
        method: "PATCH",
        headers: {
  ...
  const FetchAirTime = async () => {
    const response = await fetch("http://127.0.0.1:4000/api/v1/sms/send-airTimme");
    const json = await response.json();
    if (response.ok) {
      console.log(json);
    } else {
            ...
            body: JSON.stringify({
              responders: [
                ...data.responders,
                {
                  userId: user._id,
                  name: user.name,
                  image: `http://localhost:4000/images/${user._id}/${user.image}`,
                  state: "At the scene",
                },
              ],
            }),
  ...
  return (
    <div className="details">
      ...
      <div className="createdBy">
        Posted by {data.createdBy} , 40 minutes ago
      </div>
      {data && user && user._id === data.userId && (
      ...
      {data && user && user._id !== data.userId && data.type !== "inactive" && (
        <div className="btn">
      ...
      {data && data.type === "active" && (
        <div className="form">
          <img
            ...
            // src={`http://127.0.0.1:4000/images/${user._id}/${user.image}`}
            alt=""
          />
          <input
            type="text"
...
const Comment = ({ user }) => {
  return (
    ...
    >
      <div className="left">
        <img src={`src/assets/Avatar/${user.image}.svg`} alt="" />
      </div>
      <div className="right">
        <h2>{user.name}</h2>
